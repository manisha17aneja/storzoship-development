...
if (!function_exists('get_shiping_price')) {
...

    {
        ...
        $distance = getdistance($from_pin, $to_pin, $from_pin_data[0]->state, $to_pin_data[0]->state, $from_pin_data[0]->city, $to_pin_data[0]->city);
        ...
        if (!empty($user_state) && $user_state[0]['state'] != "") {
            ...
            file_put_contents(APPPATH . 'logs/get_shiping_price/' . date("d-m-Y") . '_log.txt', print_r($log_data7, true), FILE_APPEND);
        ...
        foreach ($logistic_rules as $key => $rules) {
            ...
            $get_sub_total_and_cod = get_price_on_rules_per_logistic($weight, $rules, $mange_price_type, $order_type, $collectable_amount, $shipment_type_previus);
            ...
            if ($is_bulk == 1) {
                ...
                $data_log['data_response_'.$sender_id] = $data_response;


                file_put_contents(APPPATH . 'logs/get_shiping_price/' . date("d-m-Y") . '_log.txt', print_r($data_log, true) . "\n------- End Log ------\n", FILE_APPEND);
    ...
    if (!function_exists('get_price_on_rules_per_logistic')) {
        ...
        function get_price_on_rules_per_logistic($physical_weight, $rules, $mange_price_type, $order_type, $collectable_amount, $shipment_type_previus = FALSE)
...

        {
            ...
            $rule_log['order_type'] = $order_type;
            ...
            if (($order_type == 1 && $shipment_type_previus !== TRUE)) {
            ...
            file_put_contents(APPPATH . 'logs/get_shiping_price/' . date("d-m-Y") . '_log.txt', print_r($rule_log1, true) . "\n------- End Rules Log ------\n", FILE_APPEND);
    ...
    if (!function_exists('get_get_with_detail_by_state')) {
...

        {
            ...
            file_put_contents(APPPATH . 'logs/get_shiping_price/' . date("d-m-Y") . '_log.txt', print_r($gst_log1, true) . "\n------- End GST Log ------\n", FILE_APPEND);
